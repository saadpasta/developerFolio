...
export default function ProjectPage(project) {
  ...
  return (
    <div
      className={isDark ? "dark-mode" : "lite-mode"}
          ...
          <div className="project-page-canvas">
            <div className="project-page-canvas-inner">
              <div className="project-page-image-container">
                ...
                />
                {"header" in project && (
                  <h1 className="project-page-header">{project.header}</h1>
                )}
              </div>
              {TextBody(project)}
...
function TextBody(project) {
  ...
  return project.text.map((text, i) => {
    if ("videos" in text) {
      const ratio = 100.0 / text.videos.length;
      return (
        <div
          className="project-page-box"
          style={{justifyContent: "center"}}
          key={project.name + "-mediabox-" + i}
        >
          {text.videos.map((video, j) => {
            return (
              <div
                className="project-page-video-card"
                style={{width: (matches ? ratio : 100) + "%"}}
              >
    ...
    if ("image" in text || "video" in text) {
      ...
      const textDiv = (
        <div
          className="project-page-text-card"
          style={{width: (matches ? 100 - ratio : 100) + "%"}}
        >
          {text.text}
        </div>
      );
      var mediaDiv = (
        <div className="project-page-box" style={{minWidth: ratio + "%"}} />
      );
      if ("image" in text) {
        const imgRef = createRef();
        mediaDiv = (
          <div
            className="project-page-image-card"
            style={{width: (matches ? ratio : 100) + "%"}}
          >
            <img
              crossOrigin={"anonymous"}
      ...
      return (
        <div
          className="project-page-box"
          key={project.name + "-mediatextbox-" + i}
        >
